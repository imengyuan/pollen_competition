...
# try calling SNPs separately - done
ref=/ohta2/meng.yuan/rumex/pollen_competition/genome/NChap1_final.fa
bam=/ohta2/meng.yuan/rumex/pollen_competition/AnalysisReady_hap1/30aFLD_hap1.sorted.rg.dedup.bam
out=/ohta2/meng.yuan/rumex/pollen_competition/vcf/30aFLD_hap1.vcf.gz
bcftools mpileup -B -I -f ${ref} ${bam} --threads 10 \
| bcftools call --threads 15 -m -O z -o ${out}

# next
ref=/ohta2/meng.yuan/rumex/pollen_competition/genome/NChap1_final.fa
bam=/ohta2/meng.yuan/rumex/pollen_competition/AnalysisReady_hap1/31cMLD_hap1.sorted.rg.dedup.bam
out=/ohta2/meng.yuan/rumex/pollen_competition/vcf/31cMLD_hap1.vcf.gz
bcftools mpileup -B -I -f ${ref} ${bam} --threads 10 \
| bcftools call --threads 15 -m -O z -o ${out}
...
do
bcftools mpileup -a AD,DP,ADF,ADR -d 5000 -Ov -f ${ref} ${in}/${i}_hap1.sorted.rg.dedup.bam --threads 20 \
 > ${out}/${i}_hap1.pileup.vcf
done
...
for i in $(less /ohta2/meng.yuan/rumex/pollen_competition/sampleSD.txt)
do
bcftools mpileup -a AD,DP,ADF,ADR -d 5000 -Ov -f ${ref} ${in}/${i}_hap2.sorted.rg.dedup.bam --threads 20 \
 > ${out}/${i}_hap2.pileup.vcf
done
