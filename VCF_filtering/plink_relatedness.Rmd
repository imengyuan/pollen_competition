...
p <- b + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)"))
ggsave(p, filename = paste0(data, "PCA_40samples.pdf"), device = "pdf")
```
...
```{r}
p1 <- ggplot(pca_FLD, aes(PC1, PC2, col = tissue)) + geom_point(size = 3, alpha = 0.6) + scale_colour_manual(values = c("red")) + coord_equal() + theme_light() + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)")) + geom_text_repel(aes(label = ind), size = 2)

p2 <- ggplot(pca_MLD, aes(PC1, PC2, col = tissue)) + geom_point(size = 3, alpha = 0.6) + scale_colour_manual(values = c("blue")) + coord_equal() + theme_light() + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)")) + geom_text_repel(aes(label = ind), size = 2)

p3 <- ggplot(pca_SD, aes(PC1, PC2, col = tissue)) + geom_point(size = 3, alpha = 0.6) + scale_colour_manual(values = c("purple")) + coord_equal() + theme_light() + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)")) + geom_text_repel(aes(label = ind), size = 2)

p4 <- ggplot(pca_PD, aes(PC1, PC2, col = tissue)) + geom_point(size = 3, alpha = 0.6) + scale_colour_manual(values = c("orange")) + coord_equal() + theme_light() + xlab(paste0("PC1 (", signif(pve$pve[1], 3), "%)")) + ylab(paste0("PC2 (", signif(pve$pve[2], 3), "%)")) + geom_text_repel(aes(label = ind), size = 2)
